...
<div class="main-div">
    <form action="index.php?controller=backend/productCategory&action=update&id=<?php echo $productCategory['id'] ?>" method="post" name="theForm" enctype="multipart/form-data" onsubmit="return validate()">
                ...
                <td>
                    <select name="parent_id">
                        ...
                        <?php foreach($productCategorys as $item): ?>
                            <option <?php if ($productCategory['parent_id'] == $item['id']): ?>selected<?php endif; ?> value="<?php echo $item['id'] ?>"><?php echo str_repeat('&nbsp;', $item['level'] * 2) . $item['name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            </tr>
